<?php include("./update_control/kakunin_Medical_control.php"); ?>
<style>
        .meddiv {
        padding: 1rem 2rem;
...
<h4>受入可能な診療内容</h4>
        ...
        <tr><th>【<b><?php echo $Dep; ?></b>】</th></tr>
                ...
                foreach($medCare_all as $Det){ 
                        if($Dep===$Det['med_dep']){ ?>
                <tr><td><label><input type="checkbox" class="uk-checkbox" disabled <?php if(isset($med_care[$Det['med_code']])) { echo 'checked'; } ?> value="1"><?php echo ($Det['med_det']);?></label><br></td></tr>
                <?php   } 
                }?>
        </table>
        <?php } ?>
...
<label class="uk-form-label">（自由入力、1000文字以内）</label></h4>
        <div class="uk-form-controls">
                <textarea class="uk-textarea size-textarea-Notes" name="mcare_note" rows="7" maxlength="1000" readonly><?php echo $mcare_note; ?></textarea>
        </div>
</div>