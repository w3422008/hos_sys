...
foreach ($medCare_data as $key =>$var): ?>
        ...
        <tr><th>【<b><?php echo $Dep; ?></b>】</th></tr>
                ...
                foreach($medCare_all as $Det){ 
                        if($Dep===$Det['med_dep']){ ?>
                <tr><td><label <?php if(!empty($medCare_data) && $var['med_'.$Det['med_code']]==='1'){ echo 'style="color:red;"'; }else{ echo 'style="color:gray; text-decoration: line-through;"'; } ?>><?php echo ($Det['med_det']);?></label><br></td></tr>
                <?php   } 
                }?>
        </table>
        <?php } ?>
...
<label class="uk-form-label">（自由入力、1000文字以内）</label></h4>
        ...
        <span><?php foreach($medCare_data as $key =>$var){ echo html_escape(trim($var['med_note'])); }?></span>
        </div>
</div>
