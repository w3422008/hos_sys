<?php
// detail_control.phpから$medical_data, $medCare_all, $medCare_naika, $medCare_geka, $med_depts等が渡されている
?>
...
<h4>受入可能な診療内容</h4>
        <?php 
        if(!empty($medical_data)){ //もしデータがあれば 
        foreach ($medical_data as $key =>$var):?>
                ...
                <tr><th>【<b><?php echo $Dep; ?></b>】</th></tr>
                        ...
                        foreach($medCare_all as $Det){ 
                                if($Dep===$Det['med_dep']){ ?>
                        <tr><td><label><input type="checkbox" class="uk-checkbox" name="med_care[<?php echo $Det['med_code']; ?>]" value="1" <?php if(!empty($medical_data) && $var['med_'.$Det['med_code']]==='1'){ echo 'checked'; } ?>><?php echo ($Det['med_det']);?></label><br></td></tr>
                        <?php   } 
                        }?>
                </table>
                <?php } ?>
...
<label class="uk-form-label">（自由入力、1000文字以内）</label></h4>
        ...
        <textarea class="uk-textarea size-textarea-Notes" name="mcare_note" rows="7" maxlength="1000"><?php foreach ($medical_data as $key =>$var){ echo html_escape(trim($var['med_note'])); } ?></textarea>
        </div>
</div>
